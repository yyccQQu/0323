...
function PlatformService(APP_CONFIG, httpSvc) {
    return {
        ...
        getAdminReset: getAdminReset, // 平台管理--初始化	DELETE/admin
        ...
        getHolderUpdate: getHolderUpdate, // 开户人管理--修改--提交	PUT /holder/updata
    ...
    function getHolderUpdate(postData) {
        return httpSvc.put(APP_CONFIG.apiUrls.HOLDER_UPDATE, postData).then(getDataComplete)
            .catch(getDataFailed);
    ...
    // 平台管理--初始化密码
    function getAdminReset(postData) {
        return httpSvc.put(APP_CONFIG.apiUrls.INIT_ADMIN, postData).then(getDataComplete)
            .catch(getDataFailed);
